<?php 
session_start();
require_once '/var/cred/cred.php';
...
function register(){
	...
	$usercountsql = mysqli_query($db, $usernamecheck);
	$emailcheck = "SELECT email from users WHERE email = '$email'";
	$emailcheckquery = mysqli_query($db, $emailcheck);
	...
	if (mysqli_num_rows($usercountsql) > 0){
...
function display_error() {
	...
	if (count($errors) > 0){
		echo "<div id='alerttest' class='alert alert-dismissible alert-danger'>
...
function login(){
	...
	if (count($errors) == 0) {
		...
		$query = "SELECT * FROM users WHERE email='$username' OR username='$username' AND password='$password' LIMIT 1";
